...
java $JAVA_OPTS -Dloader.config.path=file:$APP_PROPS -Dehcache.config.path=$ehcacheconfig -Dorg.terracotta.license.path=$license_path -cp "libs/*" $execClass $params | tee $logs_home/run.log